# TailorAd — 작업 기록 (Last Updated: 2026-09-10 22:05)
...
- **스택**: Next.js 16 + Tailwind v4 + Supabase + Replicate/OpenRouter + Upstash Redis, Cloudflare Workers 배포(OpenNext)
- **Git**: Git Flow (`master` 프로덕션 / `develop` 작업). 현재 `develop`에 4커밋 적재, master 미반영, 푸시 안 함.
...
- **테마**: 라이트가 기본 (`theme-light`), 다크는 토글. 사장님 브라우저에서 원격 폰트 차단 시 Pretendard 폴백으로 보임 (코드 문제 아님).
...
- **세션 중 추가**: `proxy.ts` (신규, 로그인 가드), `app/api/client-gateway/` (C2S→S2S),
  `app/api/user/[userId]/` (GET+PATCH, S2S+IDOR 가드), `public/logo/logo-64.png`,
  `lib/replicateRateLimit.ts`, `components/.../[projectId]/components/compositeDownload.tsx`
...
6. **린트 전멸**: 복사해온 코드의 에러 18개 수정 (진짜 버그 1개 포함: PortfolioSection hooks).
   세션 말미 재측정: 0 errors, warnings 10개(기존 잔재, 빌드 무관).
...
- 타일·모달·다운로드 PNG 3곳 일치 검증됨 ("…dress / it" 케이스).
...
## 7. 다음 작업: Remotion 에디터 (신규)

- LLM이 생성한 design을 바탕으로 요소(헤드라인/CTA/로고/scrim) 배치·편집하는 에디터를 Remotion으로 새로 만든다.
- **초기 배치 일치 원칙**: 에디터 진입 시 첫 화면의 텍스트 위치·개행이 타일/모달/다운로드와 똑같아야 한다.
  이를 위해 에디터의 좌표 해석은 §5 계약 + `AdOverlay` 구현을 그대로 따른다 (높이 기준 fontSizePct, 동일 폰트 스택·웨이트·자간, 동일 개행).
- Remotion Player 프리뷰와 최종 렌더가 같은 % 좌표계를 쓰도록 설계한다.
- 범위·스펙은 별도 세션에서 정의 (이 파일에는 원칙만 기록).
...
npm run build
npm run lint     # 현재 0 errors, warnings 10 (기존 잔재)
